...
def run_academic_pipeline(
    ...
    usable_sources = [s for s in sources if s.is_usable]
    used_usable_sources = [s for s in citation_analysis.used_sources if s.is_usable]
    has_source_deficiency = len(usable_sources) < spec.source_requirements.minimum_sources
    ...
    report = WritingReport(
        ...
        sources_retrieved=len(usable_sources),
        sources_used=len(used_usable_sources),
