...
export const SourceDetailModal: React.FC<SourceDetailModalProps> = ({
  ...
  const relevance = source.relevance || 'DIRECT';
  const getRelevanceBadge = () => {
    switch (relevance) {
      case 'DIRECT':
        return { chipClass: 'chip-pass', desc: 'Directly relevant to the assignment topic.' };
      case 'SUPPORTING':
        return { chipClass: 'chip-pass', desc: 'Supports the topic without being central.' };
      case 'TANGENTIAL':
        return { chipClass: 'chip-amber', desc: 'Marginally related; does not count toward source minimum.' };
      case 'IRRELEVANT':
        return { chipClass: 'chip-fail', desc: 'Not relevant to the assignment topic.' };
      default:
        return { chipClass: 'chip-neutral', desc: 'Relevance not classified.' };
    }
  };
      ...
      <div className="modal-card" style={{ width: '780px' }}>
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            <span className={`chip ${relBadge.chipClass}`}>RELEVANCE: {relevance}</span>
          ...
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', padding: '0.65rem 0.85rem', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
            <strong>Relevance:</strong> {relBadge.desc} <strong>Origin:</strong> {badge.desc}
          </div>
