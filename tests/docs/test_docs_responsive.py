...
def test_howlwriter_responsive_viewports(docs_server, width, height, name):
    with sync_playwright() as p:
        ...
        assert not diag["hasOverflow"], (
            f"Document overflow at {width}px: "
            f"scrollWidth {diag['scrollWidth']} > "
            f"clientWidth {diag['clientWidth']}"
        )
        assert len(diag["uncontained"]) == 0, (
            f"Uncontained elements at {width}px: {diag['uncontained']}"
        )
        assert len(console_errors) == 0, (
            f"Console errors at {width}px: {console_errors}"
        )
